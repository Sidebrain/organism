import { Button } from "./ui/button";
import { useAppContext } from "@/context/AppContext";

// Header component with title and switch sides button
const GenerativeHeader = () => {
  const appContext = useAppContext();

  const handleSwitchSides = () => {
    appContext.setSettings((prev) => ({
      ...prev,
      generativeOnRight: !prev.generativeOnRight,
    }));
  };

  return (
    <div className="p-4 border-b border-border flex justify-between items-center">
      <h2 className="text-lg font-medium">Generated Output</h2>
      <Button variant="ghost" size="sm" onClick={handleSwitchSides}>
        Switch Sides
      </Button>
    </div>
  );
};

// Component to display individual generative messages
const GenerativeMessage = ({
  message,
}: {
  message: { id: string; timestamp: Date; content: string };
}) => {
  return (
    <div className="bg-secondary/10 rounded-lg p-4">
      <p className="text-sm text-muted-foreground mb-2">
        {message.timestamp.toLocaleTimeString()}
      </p>
      <pre className="whitespace-pre-wrap text-sm bg-background border border-border rounded p-3 overflow-x-auto">
        {message.content}
      </pre>
    </div>
  );
};

// Component to display the list of generative messages
const GenerativeMessageList = () => {
  const appContext = useAppContext();

  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      {appContext.generativeMessages.map((message) => (
        <GenerativeMessage key={message.id} message={message} />
      ))}
    </div>
  );
};

// Main component that composes all the modular pieces
export const GenerativeArea = () => {
  return (
    <div className="flex flex-col h-full bg-background">
      <GenerativeHeader />
      <GenerativeMessageList />
    </div>
  );
};
